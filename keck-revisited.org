...
** STARTED [6/8] Application to data reduction pipeline
:LOGBOOK:
...
+ [X] Deconvolve doublets
+ [ ] Remove nebula
  + [ ] First, redo the fitting to scale the objective function by the pixel standard deviation. 
...
+ This is after both have been corrected for order.
  + [X] Maybe it would also work to do it before correcting the order overlap.  NO, NOT WORTH IT
